  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (customer?.id) {
        ...
        const res = await customersService.create(form)
        savedId = res.data.id
        toast.success('Cliente registrado exitosamente')
      }
      if (photoFile && savedId) {
        await customersService.uploadPhoto(savedId, photoFile)
      }
      onSaved()
    } catch (e: unknown) {
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100">
          <div className="min-w-0">
            <h2 className="text-base sm:text-lg font-bold text-gray-900 truncate">{customer ? 'Editar Cliente' : 'Nuevo Cliente'}</h2>
            <p className="text-xs sm:text-sm text-gray-400 truncate">Paso {step + 1} de {STEPS.length}: {STEPS[step]}</p>
          </div>
        ...
        {/* Step indicators */}
        <div className="flex px-4 sm:px-6 pt-3 sm:pt-4 gap-1.5 sm:gap-2 overflow-x-auto scrollbar-hide">
          {STEPS.map((s, i) => (
            <button key={s} onClick={() => setStep(i)}
              className={`flex-1 min-w-[80px] py-1.5 text-[11px] sm:text-xs font-medium rounded-lg transition-colors whitespace-nowrap ${i === step ? 'bg-primary-600 text-white' : i < step ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-400'}`}
            >{s}</button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-3 sm:py-4 space-y-4">
          {step === 0 && <StepPersonal form={form} set={set} branches={branches} photoPreview={photoPreview}
...
function StepPersonal({ form, set, branches, photoPreview, onPhotoChange }: {
  form: Partial<Customer>; set: (f: keyof Customer, v: unknown) => void; branches: Branch[]
  photoPreview: string | null; onPhotoChange: (file: File, preview: string) => void
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4">
        <div className="relative shrink-0">
          {photoPreview ? (
            <img src={photoPreview} alt="Foto" className="h-20 w-20 rounded-xl object-cover border-2 border-gray-200" />
          ) : (
            <div className="h-20 w-20 rounded-xl bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center">
        ...
        <div>
          <button type="button" onClick={() => fileRef.current?.click()}
            className="px-3 py-1.5 text-sm bg-primary-50 text-primary-700 border border-primary-200 rounded-lg hover:bg-primary-100 font-medium">
            {photoPreview ? 'Cambiar foto' : 'Subir foto'}
          </button>
          <p className="text-[10px] text-gray-400 mt-1">JPG, PNG. Máximo 5 MB</p>
